			...
			shared.getmd5(md5_file_name,shared.retailer.retailer_code).then((data) ->
				...
				if data
					...
					if ($('[class="pdp_price"] [class="price"] [class="old-price"]').html())
						...
						if($('[class="product-info-price"] [class="price price-world"]').text())
							#priceNow = $('[class="pdp_price"] [class="price"]').text().trim().replace(/\s*\([^>]*?\)$/ig,'').trim()
					...
					if($('[class="price-box price-final_price"]  [class="price "] span meta').attr('content'))
						priceNow=$('[class="price-box price-final_price"]  [class="price "] span meta').attr('content')
						priceWas=priceNow
						pricetext=priceNow
					if(priceNow=='' || priceNow==null)
						priceNew=$('[class="price-box price-final_price"] [class="price price-world sale_price"]').html()
						if(priceNew.match(/US\$(.*?)\s*<span\s*class="old-price">\(US\$(.*?)\)<\/span>/i))
							priceNow=priceNew.match(/US\$(.*?)\s*<span\s*class="old-price">\(US\$(.*?)\)<\/span>/i)[1]
							priceWas=priceNew.match(/US\$(.*?)\s*<span\s*class="old-price">\(US\$(.*?)\)<\/span>/i)[2]
							pricetext=priceNow
						else
							priceNow=''
					if(priceNow=='' || priceNow==null)
						if(data.match(/{"amount":(.*?)}/i))
							priceNow=data.match(/{"amount":(.*?)}/i)[1]
							priceWas=priceNow
							pricetext=priceNow
					...
					sku_data=''
					sku_data2=''
					if(data.match(/"\[data\-role\=swatch\-options]\"\:\s*{\s*\"[\w\W]*?":\s*{\s*"[\w\W]*?":\s*{"[\w\W]*?":{"138":\s*{"[\w\W]*?":\[(.*?)],/i))
						sku_data=data.match(/"\[data\-role\=swatch\-options]\"\:\s*{\s*\"[\w\W]*?":\s*{\s*"[\w\W]*?":\s*{"[\w\W]*?":{"138":\s*{"[\w\W]*?":\[(.*?)],/i)[1]
						if(sku_data.match(/{"id":"(.*?)","label":"(.*?)","[\w\W]*?":\[(.*?)]}/igm))
							sku_data1=(/{"id":"(.*?)","label":"(.*?)","[\w\W]*?":\[(.*?)]}/igm)
							#sku_data2=sku_data1.exec(sku_data)
							sku_data.match(/{"id":"(.*?)","label":"(.*?)","[\w\W]*?":\[(.*?)]}/igm).forEach (ele) ->
								sku_data2=ele;
								if(sku_data2.match(/label":"(.*?)"/i))
									size=sku_data2.match(/label":"(.*?)"/i)[1]
								if(sku_data2.match(/products\"\:\[(.*?)\]/i))
									stock_info=sku_data2.match(/products\"\:\[(.*?)\]/i)[1]
								stock=true
								if(stock_info=='' || stock_info==null || stock_info.match(/"\s*"/ig))
									stock=false
								unless(size)
									size = 'No Size'
								unless(color)
		...
		shared.client.smembers retailerCode, (err, result) ->
			...
			shared.common_client.select 1, (err) ->
				result.forEach (url) ->
					...
					shared.auth['headers']['Pragma']="no-cache"
					shared.auth['headers']['Cache-Control']="no-cache"
					shared.auth['headers']['Upgrade-Insecure-Requests']="1"
					shared.auth['headers']['User-Agent']="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/84.0.4147.135 Safari/537.36"
					shared.auth['headers']['Sec-Fetch-Site']="none"
					shared.auth['headers']['sec-fetch-user']="?1"
					shared.auth['headers']['sec-fetch-mode']="navigate"
					shared.auth['headers']['sec-fetch-dest']="document"
