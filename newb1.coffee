			...
			# shared.getmd5(md5_file_name).then((data) ->
				...
				if data
					...
					$('.mr-2.mr-lg-3.mb-2.mb-lg-3.variation-attribute.style-attribute').each (index, el) ->
						...
						if(color_raw.match(/\s*Select Style\s*/i))
							color_raw=color_raw.replace(/\s*Select Style\s*/ig,'')
						JSON_image_sku[image_key] = image_url1
						JSON_color_raw_sku[image_key] = color_raw
					$('.mr-2.mr-lg-3.mb-2.mb-lg-3.variation-attribute.width-attribute').each (index, el) ->
						JSON_data_sku[width_id]
						width_id = $(el).find('.width-value').text().trim()
						width_key = $(el).find('.width-value').attr('data-attr-value')
						console.log("width id "+width_id+"width key "+width_key)
						if(width_id.match(/\s*Select Width\s*/i))
							width_id=width_id.replace(/\s*Select Width\s*/ig,'')
						JSON_data_sku[width_key]=width_id
						#image_url1 = $(el).find('span').attr('style')
					...
					if(Json_data1!='' && Json_data2!='' )
						...
						Object.keys(Json_data2['variants']).forEach (sku_id1) ->
							...
							if(Json_data2['variants'][sku_id1]['width'])
								...
								width_key=JSON_data_sku[width_id]
								size = size+" "+width_key
								console.log("width"+width_key)
							...
							Object.keys(Json_data1['variants']).forEach (key,value) ->
								...
								if(sku_id2==size_id)
									...
									if(Json_data1['variants'][key]['status'].match(/IN_STOCK/i))
